...
RUN ARCH=$(uname -m) && \
    if [ "$ARCH" = "x86_64" ]; then \
        ...
        echo "Chrome is not available for ppc64le architecture. Skipping Chrome installation."; \
    fi
...
RUN ARCH=$(uname -m) && \
    ...
    if [ "$ARCH" = "x86_64" ]; then \
        ...
        echo "ChromeDriver is not available for ppc64le architecture. Skipping ChromeDriver installation."; \
    fi && \
